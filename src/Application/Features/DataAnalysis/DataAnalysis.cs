...
namespace Application.Features.DataAnalysis {
    public class DataAnalysis {
        ...
        private static Func<MergedData, ConsiderCards> MergedToConsiderSelector => d => new ConsiderCards
            {Name = d.SealedCard.Card, Cost = d.SetCard.ManaCost, Rating = d.ReviewValues.AvgScore};
        ...
        public static (IEnumerable<ConsiderCards> InColorCards, IEnumerable<ConsiderCards> SharedColorCards)
            GetCardsToConsider(IEnumerable<MergedData> deck, IEnumerable<MergedData> pool) {
            var deckColors = deck
                .Where(d => d.SetCard.Colors is not null)
